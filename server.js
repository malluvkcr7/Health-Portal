...
app.get('/medical-records', (req, res) => {
    res.sendFile(__dirname + '/public/templates/medicalrecords.html');
});
...
app.route('/appointments')
  ...
  .post(async (req, res) => {
    ...
    patientspecialist=req.body.specialist;
     // Assuming username is passed as a form field
    ...
    try {
      ...
      console.log(saved);
      res.redirect(`/medical-records`); // Redirect to the appointments page after submission
    } catch (error) {
  ...
  app.post('/save-personal-info', async (req, res) => {
    ...
    try {
        ...
        await newPersonalInfo.save();
        res.redirect('/success'); // Redirect to the medical records page or another relevant page
    } catch (error) {
